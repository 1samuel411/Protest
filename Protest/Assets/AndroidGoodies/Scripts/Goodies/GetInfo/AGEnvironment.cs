...
namespace DeadMosquito.AndroidGoodies
{
    ...
    {
        private const string EnvClass = "android.os.Environment";
        ...
        private static string GetFileDirectory(string methodName, params object[] args)
        {
            ...
            try
            {
                using (var env = new AndroidJavaClass(EnvClass))
                {
            ...
            {
                Debug.LogWarning(string.Format("Failed to get directory {0} on {1}, Error: {2}", methodName, EnvClass, e.Message));
                return null;
            }
        }
        ...
        private static T EnvClassCallStatic<T>(string methodName, params object[] args)
        {
            ...
            try
            {
                return EnvClass.AJCCallStaticOnce<T>(methodName, args);
            }
            catch (Exception e)
            {
                Debug.LogWarning(string.Format("Failed to invoke {0} on {1}, Error: {2}", methodName, EnvClass, e.Message));
                return default(T);
            }
        }
